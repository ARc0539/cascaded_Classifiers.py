...
#-- 1. Load the cascades
if not feature_cascade.load(cv.samples.findFile(building_cascade_name)):
    print('--(!)Error loading building cascade')
    exit(0)
if not eyes_cascade.load(cv.samples.findFile(building_cascade_name)):
    print('--(!)Error loading building cascade')
    exit(0)
